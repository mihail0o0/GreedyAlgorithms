function test(uloge: string[], k: number): number {
    let broj = 0;
    let lopovi: number[] = [];
    ...
    for (let i = 0; i < uloge.length; i++) {
        if(uloge[i] != 'p') continue; // gledamo samo za policajce

        // trazimo sleva na desno
        for (let j = -k; j <= k; j++) {
            if (i + j < 0 || i + j >= uloge.length) continue; // ako izlazimo van granica niza
